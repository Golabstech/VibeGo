...
esp_lcd_touch_handle_t touch_gt911_init()
{
    ...
    CH422G_init();  // Initialize the CH422G GPIO chip for backlight control
    ...
    CH422G_io_output(CH422G_IO_5, 0);
    CH422G_io_output(CH422G_IO_1, 0);  // Set GPIO for backlight control to low (off)
    
    vTaskDelay(pdMS_TO_TICKS(100));  // Wait for 100ms
    digitalWrite(EXAMPLE_PIN_NUM_TOUCH_INT, 0);  // Set interrupt pin low (disable interrupt)
    
    vTaskDelay(pdMS_TO_TICKS(100));  // Wait for another 100ms
    CH422G_io_output(CH422G_IO_1, 1);  // Set GPIO for backlight control to high (on)
    ...
    ESP_ERROR_CHECK(esp_lcd_new_panel_io_i2c((esp_lcd_i2c_bus_handle_t)EXAMPLE_I2C_MASTER_NUM, &tp_io_config, &tp_io_handle)); // Create new I2C panel I/O
