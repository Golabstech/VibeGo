/**
 * ============================================================
 * VibeGo Kiosk - UI Implementation (Waveshare 5")
 * Platform: ESP32-S3-Touch-LCD-5B (1024x600 Landscape)
...
static void safe_scr_load_with_cleanup(lv_obj_t * new_scr) {
    ...
    // Prevent loading same screen
    if (lv_scr_act() == new_scr) {
         printf("[UI] Transition blocked (same screen)\n");
         return;
    }
...
static void ui_event_disclaimer_btn(lv_event_t * e) {
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ui_Home_screen_init(); // Re-create before load
...
static void ui_event_start_btn(lv_event_t * e) {
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ui_Measuring_screen_init(); // Re-create before load
        safe_scr_load(ui_Measuring, LV_SCR_LOAD_ANIM_NONE, 0, 0, true);
    }
}

static void ui_event_new_test_btn(lv_event_t * e) {
    if (lv_event_get_code(e) == LV_EVENT_CLICKED) {
        ui_Disclaimer_screen_init(); // Re-create before load
        safe_scr_load(ui_Disclaimer, LV_SCR_LOAD_ANIM_NONE, 0, 0, true);
    }
}

// Splash screen timer callback
static void splash_timer_cb(lv_timer_t * timer) {
    splash_progress += 2; // Slower progress for larger screen effect
    ...
    if (splash_progress >= 100) {
        lv_timer_del(timer);
        splash_timer = NULL;
        // Direct switch without animation to prevent freeze
        ui_Disclaimer_screen_init(); // Re-create
        safe_scr_load(ui_Disclaimer, LV_SCR_LOAD_ANIM_NONE, 0, 0, true);
    }
}
...
void ui_Splash_screen_init(void) {
    ...
    lv_obj_align(ui_Splash_Logo, LV_ALIGN_CENTER, 0, -60);

    // Subtitle
    ui_Splash_Subtitle = lv_label_create(ui_Splash);
    lv_label_set_text(ui_Splash_Subtitle, "Developed by Golabs A.S.");
    lv_obj_set_style_text_font(ui_Splash_Subtitle, &lv_font_montserrat_24, 0); // 12->24
    lv_obj_set_style_text_color(ui_Splash_Subtitle, lv_color_hex(UI_COLOR_TEXT_MUTED), 0);
    lv_obj_align(ui_Splash_Subtitle, LV_ALIGN_CENTER, 0, 80);

    // Version
    ui_Splash_Version = lv_label_create(ui_Splash);
    lv_label_set_text(ui_Splash_Version, "v1.4.0 (Waveshare)");
    lv_obj_set_style_text_font(ui_Splash_Version, &lv_font_montserrat_20, 0); // 10->20
    lv_obj_set_style_text_color(ui_Splash_Version, lv_color_hex(UI_COLOR_TEXT_MUTED), 0);
    lv_obj_align(ui_Splash_Version, LV_ALIGN_BOTTOM_MID, 0, -80);

    // Progress bar
    ui_Splash_Progress = lv_bar_create(ui_Splash);
    lv_obj_set_size(ui_Splash_Progress, 600, 20); // 200->600, 8->20
    lv_obj_align(ui_Splash_Progress, LV_ALIGN_BOTTOM_MID, 0, -120);
    ...
    lv_obj_set_style_radius(ui_Splash_Progress, 10, LV_PART_MAIN); 
    lv_obj_set_style_radius(ui_Splash_Progress, 10, LV_PART_INDICATOR);

    // Start progress timer
    splash_progress = 0;
    splash_timer = lv_timer_create(splash_timer_cb, 50, NULL); // Faster update, smoother anim
}
...
void ui_Disclaimer_screen_init(void) {
    ...
    lv_obj_set_style_bg_opa(ui_Disclaimer, 255, 0); // Force Opaque

    ui_Disclaimer_Panel = lv_obj_create(ui_Disclaimer);
    lv_obj_set_size(ui_Disclaimer_Panel, 800, 450); // 300x200 -> 800x450
    ...
    lv_obj_set_style_border_width(ui_Disclaimer_Panel, 2, 0);
    lv_obj_set_style_radius(ui_Disclaimer_Panel, 24, 0);
    lv_obj_set_style_pad_all(ui_Disclaimer_Panel, 30, 0);
    ...
    lv_obj_set_style_text_font(ui_Disclaimer_Title, &lv_font_montserrat_36, 0); // 20->36
    lv_obj_align(ui_Disclaimer_Title, LV_ALIGN_TOP_MID, 0, 10);

    ui_Disclaimer_Text = lv_label_create(ui_Disclaimer_Panel);
    lv_label_set_long_mode(ui_Disclaimer_Text, LV_LABEL_LONG_WRAP);
    lv_obj_set_width(ui_Disclaimer_Text, 700); // 270->700
    lv_label_set_text(ui_Disclaimer_Text, 
        "DIKKAT: Bu cihaz yasal veya tibbi olcum yapmaz.\n"
        "Sonuclar sadece tahminidir ve eglence amacli kullanilmalidir.\n"
        "Alkol aldiysaniz lutfen ARAC KULLANMAYIN!");
    lv_obj_set_style_text_color(ui_Disclaimer_Text, lv_color_hex(UI_COLOR_TEXT_BODY), 0);
    lv_obj_set_style_text_font(ui_Disclaimer_Text, &lv_font_montserrat_24, 0); // 12->24
    lv_obj_set_style_text_align(ui_Disclaimer_Text, LV_TEXT_ALIGN_CENTER, 0);
    lv_obj_align(ui_Disclaimer_Text, LV_ALIGN_CENTER, 0, -20); // Biraz yukarı

    ui_Disclaimer_Button = lv_btn_create(ui_Disclaimer_Panel);
    lv_obj_set_size(ui_Disclaimer_Button, 400, 80); // 200x40 -> 400x80
    lv_obj_align(ui_Disclaimer_Button, LV_ALIGN_BOTTOM_MID, 0, -20);
    ...
    lv_obj_set_style_radius(ui_Disclaimer_Button, 40, 0);
    lv_obj_set_style_shadow_width(ui_Disclaimer_Button, 20, 0);
    ...
    lv_obj_set_style_text_font(btn_label, &lv_font_montserrat_28, 0); // 14->28
...
void ui_Home_screen_init(void) {
    ...
    lv_obj_set_style_bg_opa(ui_Home, 255, 0); // Force Opaque

    // LEFT PANEL (Logo, Slogan, Main Action)
    lv_obj_t * left_panel = lv_obj_create(ui_Home);
    lv_obj_set_size(left_panel, 500, 500);
    ...
    lv_obj_set_style_text_font(ui_Home_Logo, &lv_font_montserrat_48, 0); // 20->48
    lv_obj_set_style_text_color(ui_Home_Logo, lv_color_hex(UI_COLOR_TEXT_PRIMARY), 0);
    lv_obj_set_style_text_letter_space(ui_Home_Logo, 5, 0);
    lv_obj_align(ui_Home_Logo, LV_ALIGN_TOP_MID, 0, 40);

    ui_Home_Subtitle = lv_label_create(left_panel);
    lv_label_set_text(ui_Home_Subtitle, "Check Your Vibe");
    lv_obj_set_style_text_font(ui_Home_Subtitle, &lv_font_montserrat_24, 0); // 12->24
    lv_obj_set_style_text_color(ui_Home_Subtitle, lv_color_hex(UI_COLOR_TEXT_MUTED), 0);
    lv_obj_align(ui_Home_Subtitle, LV_ALIGN_TOP_MID, 0, 100);

    // Decorative Pulse Ring (background for button)
    lv_obj_t * pulse_ring = lv_obj_create(left_panel);
    lv_obj_set_size(pulse_ring, 420, 160);
    lv_obj_align(pulse_ring, LV_ALIGN_CENTER, 0, 50);
    lv_obj_set_style_bg_color(pulse_ring, lv_color_hex(UI_COLOR_PRIMARY), 0);
    lv_obj_set_style_bg_opa(pulse_ring, 40, 0);
    lv_obj_set_style_radius(pulse_ring, 80, 0);
    lv_obj_set_style_border_width(pulse_ring, 0, 0);

    ui_Home_Status_Btn = lv_btn_create(left_panel);
    lv_obj_set_size(ui_Home_Status_Btn, 400, 140);
    lv_obj_align(ui_Home_Status_Btn, LV_ALIGN_CENTER, 0, 50);
    ...
    lv_obj_set_style_border_width(ui_Home_Status_Btn, 4, 0);
    lv_obj_set_style_radius(ui_Home_Status_Btn, 70, 0);
    ...
    lv_obj_set_size(right_panel, 350, 450);
    lv_obj_align(right_panel, LV_ALIGN_RIGHT_MID, -60, 0);
    ...
    lv_obj_set_style_border_width(right_panel, 3, 0);
    ...
    lv_obj_set_style_text_font(instr_title, &lv_font_montserrat_28, 0);
    ...
    lv_obj_set_style_text_font(ui_Home_Instructions, &lv_font_montserrat_24, 0);
    lv_obj_set_style_text_line_space(ui_Home_Instructions, 10, 0);
    lv_obj_align(ui_Home_Instructions, LV_ALIGN_CENTER, 0, 30);
}
...
void ui_Measuring_screen_init(void) {
    ...
    lv_obj_set_style_bg_opa(ui_Measuring, 255, 0); // Force Opaque

    // Spinner on LEFT side (LARGE)
    ui_Measuring_Spinner = lv_spinner_create(ui_Measuring, 1000, 60);
    lv_obj_set_size(ui_Measuring_Spinner, 250, 250); // 100 -> 250
    lv_obj_set_pos(ui_Measuring_Spinner, 150, 150);
    ...
    lv_obj_set_style_arc_width(ui_Measuring_Spinner, 25, LV_PART_INDICATOR);
    lv_obj_set_style_arc_width(ui_Measuring_Spinner, 25, LV_PART_MAIN);

    // Main text on RIGHT side
    ui_Measuring_Text = lv_label_create(ui_Measuring);
    lv_label_set_text(ui_Measuring_Text, "OLCULUYOR...");
    lv_obj_set_style_text_font(ui_Measuring_Text, &lv_font_montserrat_48, 0);
    lv_obj_set_style_text_color(ui_Measuring_Text, lv_color_hex(UI_COLOR_TEXT_PRIMARY), 0);
    lv_obj_set_style_text_letter_space(ui_Measuring_Text, 2, 0);
    lv_obj_set_pos(ui_Measuring_Text, 500, 200);
    ...
    lv_obj_set_pos(hint, 500, 280);

    // Progress bar at BOTTOM (Wide)
    ui_Measuring_Progress = lv_bar_create(ui_Measuring);
    lv_obj_set_size(ui_Measuring_Progress, 800, 40);
    lv_obj_align(ui_Measuring_Progress, LV_ALIGN_BOTTOM_MID, 0, -60);
    ...
    lv_obj_set_style_radius(ui_Measuring_Progress, 20, LV_PART_MAIN);
    lv_obj_set_style_radius(ui_Measuring_Progress, 20, LV_PART_INDICATOR);
}
...
void ui_Result_screen_init(void) {
    ...
    lv_obj_set_style_text_font(ui_Result_Status_Label, &lv_font_montserrat_32, 0);
    lv_obj_set_style_text_color(ui_Result_Status_Label, lv_color_hex(UI_COLOR_GREEN), 0);
    lv_obj_align(ui_Result_Status_Label, LV_ALIGN_TOP_MID, 0, 40);
    ...
    lv_obj_set_size(ui_Result_NewTest_Btn, 250, 60);
    ...
    lv_obj_set_style_radius(ui_Result_NewTest_Btn, 30, 0);
...
void ui_init(void) {
    ...
    // Only Init Splash. Others are created on demand to save RAM.
    ui_Splash_screen_init();
    // ui_Disclaimer_screen_init(); // Created on demand
    // ui_Home_screen_init();       // Created on demand
    // ui_Measuring_screen_init();  // Created on demand
    // ui_Result_screen_init();     // Created on demand
