...
void ESP_PanelLcd_GC9A01::init(void)
{
    ...
    CHECK_ERROR_RETURN(esp_lcd_new_panel_gc9a01(bus->getHandle(), &config.dev_config, &handle));
}
...
static esp_err_t panel_gc9a01_init(esp_lcd_panel_t *panel)
{
    ...
    esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, &gc9a01->madctl_val, 1);
    esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD, &gc9a01->colmod_cal, 1);
