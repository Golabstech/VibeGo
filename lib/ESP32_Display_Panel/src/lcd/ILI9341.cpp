...
static esp_err_t esp_lcd_new_panel_ili9341(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel);
...
void ESP_PanelLcd_ILI9341::init(void)
{
    ...
    CHECK_ERROR_RETURN(esp_lcd_new_panel_ili9341(bus->getHandle(), &config.dev_config, &handle));
}
...
static esp_err_t panel_ili9341_del(esp_lcd_panel_t *panel)
{
    ...
    if (ili9341->reset_gpio_num >= 0) {
        gpio_reset_pin((gpio_num_t)ili9341->reset_gpio_num);
    }
...
static esp_err_t panel_ili9341_reset(esp_lcd_panel_t *panel)
{
    ...
    if (ili9341->reset_gpio_num >= 0) {
        gpio_set_level((gpio_num_t)ili9341->reset_gpio_num, ili9341->reset_level);
...
static esp_err_t panel_ili9341_init(esp_lcd_panel_t *panel)
{
    ...
    esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD, &ili9341->colmod_cal, 1);
