...
static esp_err_t esp_lcd_new_panel_st7796(const esp_lcd_panel_io_handle_t io, const esp_lcd_panel_dev_config_t *panel_dev_config, esp_lcd_panel_handle_t *ret_panel);
...
void ESP_PanelLcd_ST7796::init(void)
{
    ...
    CHECK_ERROR_RETURN(esp_lcd_new_panel_st7796(bus->getHandle(), &config.dev_config, &handle));
}
...
static esp_err_t panel_st7796_init(esp_lcd_panel_t *panel)
{
    ...
    esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL, &st7796->madctl_val, 1);
    esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD, &st7796->colmod_cal, 1);
